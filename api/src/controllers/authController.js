...
const login = async (req, res) => {
  ...
  dbConn.query(
    "SELECT `id`, `name`, `email`,`role`, `created_at`, `password`  FROM `patient` WHERE `email` = ? AND `password`=?",
    [email, password],
      ...
      if (results === undefined || results.length > 0) {
        ...
        if (!name || !email)
          return res
            ...
            .send({
              error: true,
              message: "Please provide user userName and email",
            });

        // insert to db
        dbConn.query(
          "INSERT INTO patient (name ,email,password,contact_no) VALUES (? ,? ,? ,?)",
